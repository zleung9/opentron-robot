...
def generate_training_set(
    ...
    if stock_solution_indices: 
        assert stock_solution_indices, "stock_solution_indices must be provided if stock_solution_only is True."
    ...
    for i in range(num_recipe):
        ...
        if stock_solution_indices: # create recipes for stock solutions only
            percentage_composition = [0] * total_num_chemical
            percentage_composition[i] = 1
...
def get_new_recipes(
    ...
    if new_ml_ids: # there are new recipes in machine learning data
        ...
        no = input("Generating training set? [Enter] to proceed or type 'no' to exit.\n>>")
        if no:
            return pd.DataFrame()
        option = input("Choose the following options: \n 1. Measure stock solutions.\n2. Generate random training set.\n>>") 
        if option == 1:
            stock_solution_indices = input("Please provide the indices (1~16) of stock solutions to measure, separated by comma. \n>>").split(",")
